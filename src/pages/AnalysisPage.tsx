...
const AnalysisPage: React.FC = () => {
  const { gameState, resetGame, setCurrentPage, backgroundImage, saveAnalysisResult } = useGameStore();
    ...
    const loadAnalysis = async () => {
      ...
      try {
        ...
        if (gameState?.phase === 'analysis') {
          ...
          else if ('overall_evaluation' in apiResponse || 'good_points' in apiResponse || 'improvements' in apiResponse) {
            console.log('Setting API analysis data directly:', apiResponse);
            setApiAnalysis(apiResponse as unknown as APIAnalysisResponse);
          }
          else {
        ...
        const totalScore = gameState?.choices?.reduce((sum, choice) => sum + choice.safetyRating, 0) || 0;
        const maxScore = (gameState?.choices?.length || 1) * 5;
        const percentage = Math.round((totalScore / maxScore) * 100);
        const calculatedGrade = getSafetyGrade(percentage);
  ...
  const getGradeColor = (grade: string) => {
    if (grade === 'A') return 'text-green-100 bg-green-500/30 border border-green-400/50';
    if (grade === 'B') return 'text-blue-100 bg-blue-500/30 border border-blue-400/50';
    if (grade === 'C') return 'text-yellow-100 bg-yellow-500/30 border border-yellow-400/50';
    if (grade === 'D') return 'text-orange-100 bg-orange-500/30 border border-orange-400/50';
    return 'text-red-100 bg-red-500/30 border border-red-400/50';
  };
  ...
  const choiceScoreSeries = gameState?.choices?.map((_, index) => {
    // 현재 선택까지의 누적 평균 계산
